...
import libsql
import numpy as np
from elfeed_summary_db_server.migrations import run_migrations
from elfeed_summary_db_server.models.db_models import SCHEMA
...
class Database:
    ...
    def __init__(self, main_path: Path, semantic_path: Path):
        ...
        if is_new_main:
            ...
            from org_db_server.migrations import (MIGRATIONS,
                                                  get_schema_version,
                                                  set_schema_version)
            if get_schema_version(self.main_conn) == 0 and MIGRATIONS:
                ...
                logger.info(
                    f"New main database created with schema version {latest_version}"
                )
        else:
        ...
        logger.info(
            f"Initialized multi-database: main={main_path.name}, semantic={semantic_path.name}"
        )
    ...
    def get_or_create_file_id(self, filename: str, md5: str,
                              file_size: int) -> int:
        ...
        if row:
            ...
            cursor.execute(
                """UPDATE files SET md5 = ?, last_updated = ?, file_size = ?, indexed_at = ?
                   WHERE rowid = ?""",
                (md5, datetime.now().isoformat(), file_size,
            ...
            cursor.execute(
                """INSERT INTO files (filename, md5, last_updated, file_size, indexed_at)
                   VALUES (?, ?, ?, ?, ?)""",
                (filename, md5, datetime.now().isoformat(), file_size,
    ...
    def populate_fts(self, filename_id: int, filename: str, content: str):
        ...
        cursor.execute("DELETE FROM fts_content WHERE filename = ?",
                       (filename, ))
    ...
    def get_or_create_linked_file(
            ...
            conversion_error: Optional[str] = None) -> int:
        ...
        if row:
            ...
            if old_md5 != md5 or old_status != conversion_status:
                cursor.execute(
                    """UPDATE linked_files
                       ...
                       WHERE rowid = ?""",
                    (md5, file_size, now, conversion_status, conversion_error,
                     now, linked_file_id))
                logger.debug(
                    f"Updated linked file {file_path} (id={linked_file_id})")
            ...
            cursor.execute(
                ...
                (org_file_id, org_link_line, file_path, file_type, file_size,
                 md5, now, conversion_status, conversion_error, now))
            linked_file_id = cursor.lastrowid
            logger.debug(
                f"Created linked file entry for {file_path} (id={linked_file_id})"
            )
            return linked_file_id
    ...
    def get_linked_files_for_org_file(self, org_file_id: int) -> List[Dict]:
        ...
        cursor.execute(
            """SELECT lf.*
               ...
               ORDER BY lf.org_link_line""", (org_file_id, ))
    ...
    def store_linked_file_chunks(self, org_filename: str, org_link_line: int,
                                 linked_file_path: str, chunks: List[Dict],
        ...
        cursor.execute("DELETE FROM chunks WHERE linked_file_path = ?",
                       (linked_file_path, ))
        ...
        self.semantic_conn.commit()
        logger.debug(
            f"Stored {len(chunks)} chunks for linked file {linked_file_path}")
